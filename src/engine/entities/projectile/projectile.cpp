...
Projectile::Projectile(const ProjectileData& pData, SDL_Point origin, SDL_FPoint originOffset, SDL_Point end, SDL_FPoint endOffset) : Entity(origin){
    ...
    this->trail = nullptr;
    this->impact = nullptr;
    if(pData.impact){ this->impact = new Particle(origin, startPixel, *pData.impact);}
    if(pData.trail){ this->trail = new Particle(origin, startPixel, *pData.trail); this->trail->play();}
...
Projectile::~Projectile(){
    ...
    if(this->trail) delete this->trail;
...
void Projectile::move(){
    ...
    float speedX = (distX / dist) * this->moveSpeed * DELTA_TIME;
    float speedY = (distY / dist) * this->moveSpeed * DELTA_TIME;

    if(this->trail){
        this->trail->move(this->gridPos, {currentPixelPosition.x, currentPixelPosition.y - this->z});
        this->trail->setSpeed(speedX, speedY);
    }
    ...
    if(
        ...
        this->gridOffset.x -= speedX;
        this->gridOffset.y -= speedY;
    } else {
...
void Projectile::land(){
    ...
    if(this->impact){ 
        SDL_FPoint current = tiles[this->gridPos.y*BOARD_WIDTH + this->gridPos.x].tile.surface[0];
        SDL_FPoint currentPixelPosition = {current.x + this->gridOffset.x, current.y + this->gridOffset.y};
        this->impact->move(this->gridPos, currentPixelPosition);
        this->impact->play();
    }
...
void sortProjectiles(SDL_GPUDevice* renderer){
    ...
    { 
        ...
        SDL_FPoint points_p[4] = {
            {ptr.x - p->quadWidth, ptr.y - p->quadHeight - p->z},
            {ptr.x + p->quadWidth, ptr.y - p->quadHeight - p->z},
            {ptr.x - p->quadWidth, ptr.y - p->z},
            {ptr.x + p->quadWidth, ptr.y - p->z},
        };
